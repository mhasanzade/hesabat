...
! function (t, i, e, n) {
    var r = function (i, e) {
        this.elem = i, this.$elem = t(i), this.options = e, this.currentData = []
    };
    r.prototype = {
        defaults: {
            containerClass: "",
        ...
        pc_remove: function (i) {
            var e = t(i.target),
                n = e.parent().data("id"),
                r = e.parent().data("order"),
                s = t("<li>").html(e.parent().text()).attr("data-id", n).attr("data-order", r);
            ...
            }), this.$elem.find(" option[value='" + n + "']").removeAttr("selected"), e.parent().remove(), this.$elem.trigger("sp-change")
        },
        pc_search: function (i) {
            var e = t(i.target).val().toLowerCase(),
                n = this._filterData(e);
            this._updateList(n, e)
        },
        _selectElement: function (t, i) {
            if (i == n && (i = this.$container.find('.pc-list li[data-id="' + t + '"]'), 0 == i.length)) return void console.log("Picker - ID to select not found!");
            if (this.config.multiple) {
                this.$container.prepend(this._createElement(i)), i.remove();
            ...
            } else this.$elem.find("option").removeAttr("selected"), this.config.coloring[t] ? this.$container.find(".pc-trigger").removeClass().addClass(this.config.coloring[selectedId] + " pc-trigger pc-element").contents().first().replaceWith(i.text()) : this.$container.find(".pc-trigger").contents().first().replaceWith(i.text());
        ...
        _insertIntoCurrentData: function (i) {
            ...
            var s;
            for (s = 0; s < this.currentData.length; s++)
                if (0 == s) {
        ...
        _createElement: function (i) {
            var e = this.config.coloring[i.data("id")],
                n = t("<span>").addClass("pc-element " + (e ? e : "")).text(i.text()).attr("data-id", i.data("id")).attr("data-order", i.data("order"));
            return n.append(t('<span class="pc-close"></span>').click(this.pc_remove.bind(this))), n
        },
        _build: function () {
            var i = this.config.texts.trigger;
            this.$container = t("<div class='picker" + (this.config.containerClass ? " " + this.config.containerClass : "") + "'><span class='pc-select' id='pc-select'><span class='pc-element pc-trigger'>" + i + "</span><span class='pc-list' " + (this.config.width ? "style='width:" + this.config.width + "px; display:none;'" : "style='display:none;'") + "><ul></ul></span></span></div>"), this.config.containerWidth !== !1 && this.$container.width(this.config.containerWidth), this.$container.insertAfter(this.$elem), this.config.search && this._buildSearch()
        },
        _buildSearch: function () {
            var i = t("<input type='search' placeholder='" + this.config.texts.search + "'>");
            i.on("input", this.pc_search.bind(this)), i.on("keypress", function (i) {
                if (13 == i.which) {
                    var e = t(i.target).val().toLowerCase(),
                        n = this._filterData(e);
                ...
                return !0
            }.bind(this)), this.$container.find(".pc-list").prepend(i)
        },
        _fillList: function () {
            var i = this.$container.find(".pc-list ul"),
                e = 0;
            this.$elem.find("option:not([hidden])").each(function (n, r) {
                var s = t("<li>").html(t(r).text()).attr("data-id", t(r).attr("value")).attr("data-order", e);
                s.click(this.pc_selected.bind(this)), i.append(s), this.config.search && this.currentData.push({
                    id: t(r).attr("value"),
                    text: t(r).text(),
                    order: e
                }), "selected" == t(r).attr("selected") && s.click(), e++
        ...
        _updateList: function (i, e) {
            ...
            for (s = 0; s < i.length; s++) {
                ...
                var o = t("<li>").html(a).attr("data-id", i[s].id).attr("data-order", i[s].order);
                o.click(this.pc_selected.bind(this)), r.append(o)
            }
        },
        api: function (t) {
            return r.prototype["api_" + t[0]] ? this["api_" + t[0]](t.slice(1)) : void console.log("Picker - unknown command!")
        },
    ...
    t.fn.picker = function (i) {
        var e = arguments;
        if (1 == t(this).length) {
            var n = t(this).data("plugin_picker");
            return n ? n.api(Array.prototype.slice.call(e)) : (t(this).data("plugin_picker", new r(this, i).init()), this)
        }
        return this.each(function () {
            var n = t(this).data("plugin_picker");
            n ? n.api(Array.prototype.slice.call(e)) : t(this).data("plugin_picker", new r(this, i).init())
        })
    }
